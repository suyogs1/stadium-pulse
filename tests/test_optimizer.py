...
from config import NARENDRA_MODI_STADIUM, StadiumState, StadiumConfig, Section, Gate
...
def test_goal_initialization_trace(agent):
    ...
    assert any("Preserving the Incentive Budget" in t for t in trace)
...
def test_no_alternative_gates_available():
    ...
    assert "No alternative gates available" in result["reasoning_trace"][-1]
